    ...
    auto Algorithm<T>::populate_new_items(const std::vector<Record<T>> &na, const std::vector<Record<T>> &oa) {
        ...
        for (const auto &record : na) {
            ...
            if (record.index() == NotFound) {
                ...
                /*
                 * We presume that the line is unchanged if one of
                 * the following conditions are met:
                 *
                 * - The current record (NA[i]) is identical with
                 *   with the entry of OA that corresponds to the position j
                 *   where additions of NA precede j.
                 *
                 *   e.g., A B C -> E D B C
                 *
                 * - We have NA[i] = i.
                 */
                if (record == oa[j] || (i == record.index() &&
                        record == oa[record.index()])) {
